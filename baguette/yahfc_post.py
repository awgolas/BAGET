...
class PostProcessing:
    ...
    def __init__(self, data_dir, inp_dict):
    ...
    def xs_collect(self):
        ...
        if self.uq_dict['mode'] == 'gls':
            path_format = os.path.join(path_format, '*')
    ...
    def gls_array_params(self):

        engy_data = self.inp_dict.get('proj_e_file')
        if engy_data is not None:
            with open(engy_data) as f:
                lines = f.readlines()
                
            engy_data = open(engy_data).readlines()
    
    def xs_gls_map(self, dir_list):

        uq_dict = self.inp_dict['uq']

        num_params = len(list(self.uq_dict['parameters'].keys()))
        n_samples = self.uq_dict['samples_per_bin']
